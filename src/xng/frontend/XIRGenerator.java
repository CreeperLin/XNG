...
public class XIRGenerator extends XASTBaseVisitor implements XASTVisitor {
    ...
    public void visitFuncDeclNode(XASTFuncDeclNode node){
        ...
        if (node.isMember){
            ...
            curThisAddr = XIRInstAddr.newStackAddr(8);
...
//                XIRInst add_inst = new XIRInst(XIRInst.opType.op_add);
                XIRInstAddr idx_addr = XIRInstAddr.newMemAddr(node.exprList.get(0).instAddr,node.exprList.get(1).instAddr,8,8);
...
//                visitLogicalExprNode(node,tn,fn);
            ...
            case e_mem: {
                visitExpr(node.exprList.get(0));
                node.instList.addAll(node.exprList.get(0).instList);
                if (node.exprList.get(1).instAddr.type == XIRInstAddr.addrType.a_imm){
...
//                node.instList.add(mv_inst);
    ...
    public void visitPrimNode(XASTPrimNode node){
        ...
        switch (node.nodeID){
            case p_id:
                ...
                if (curThisAddr != null && node.instAddr.type == XIRInstAddr.addrType.a_imm){
    ...
    public void visitCreatorNode(XASTCreatorNode node){
        ...
        if (node.ctype.dim > 0){
            ...
            {
                XIRInstAddr tmp_reg = XIRInstAddr.newRegAddr();
